#!/usr/bin/env python3
"""
Generate diamond_rank.apng — 16:9 animated APNG of the Diamond Rank pattern.
Based on .job-tier-diamond CSS from PublicJobDetail.vue:
...
W, H      = 1920, 1080
FRAMES    = 60
FPS       = 30       # delay = 1/30 s per frame  (2-second seamless loop)
SPACING   = 96       # crystal lattice spacing in pixels
STAR_R    = 16       # sparkle star radius
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR)
OUT_PATH  = os.path.join(PROJECT_ROOT, "diamond_rank.apng")

# ── CSS color stops ────────────────────────────────────────────────────────
# linear-gradient(135deg, #a8edff 0%, #7dd3fc 40%, #c7f2ff 70%, #b2e8ff 100%)
...
diag_px  = x_g + y_g                                  # (H,W) sum used throughout
t_diag   = diag_px / float(W + H - 1)                 # 0→1 along 135° diagonal
...
# ── Sparkle: pre-compute 4-pointed star kernel ────────────────────────────
...
).astype(np.float32)[..., np.newaxis]                  # (2R+1, 2R+1, 1)

# ── Sparkle: node positions (lattice intersections) ───────────────────────
half = SPACING // 2
sparkle_pts = [
    (sx, sy)
...
for fi in range(FRAMES):
    ...
    img = base.copy()     # (H,W,3) float32

    # ── Shimmer 1: wide primary (travels full diagonal over one loop) ──────
    s1_pos  = ((phase * 1.5) - 0.25) * (W + H)
    s1_dist = diag_px - s1_pos
    s1      = np.exp(-(s1_dist / 320.0) ** 2)[..., np.newaxis].astype(np.float32)
    img     = img + (1.0 - img) * (s1 * 0.42)

    # ── Shimmer 2: narrow bright secondary (0.65× speed, overtone) ────────
    ...
    # ── Shimmer 3: prismatic cross-beam (perpendicular, slight color tint) ─
    # Cross-diagonal: (W-1-x + y) for ~-45° beam
    cross_d = (W - 1 - x_g + y_g)
    s3_pos  = (((phase * 0.45 + 0.33) % 1.0) * 1.5 - 0.25) * (W + H)
    s3_dist = cross_d - s3_pos
    s3      = np.exp(-(s3_dist / 110.0) ** 2).astype(np.float32)
    # Prismatic tint: slightly cyan-violet hue
    prism_hue  = 0.5 + 0.5 * math.sin(phase * 2 * math.pi)
    prism_color = np.array([
        0.85 + 0.15 * prism_hue,
        0.95,
        1.00,
    ], dtype=np.float32)
    img = img + (prism_color - img) * (s3[..., np.newaxis] * 0.30)

    # ── Crystal lattice ────────────────────────────────────────────────────
    img = img + lattice * (1.0 - img)

    # ── Global node glow pulse ─────────────────────────────────────────────
    node_pulse = (0.5 + 0.5 * math.sin(phase * 2 * math.pi * 2 - math.pi / 2)) ** 2
    img        = img + (node_mask * node_pulse * 0.45)[..., np.newaxis] * (1.0 - img)

    # ── Per-node sparkle stars ─────────────────────────────────────────────
