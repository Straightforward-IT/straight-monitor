...
const BewerberSchema = new mongoose.Schema(
	{
		...
		status: {
			type: String,
			enum: ['new', 'in_review', 'interview', 'offer', 'hired', 'rejected', 'archived'],
			default: 'new',
			index: true,
		},
		quelle: { type: String, trim: true, default: 'asana' },
		...
		anstellungsverhaeltnis: { type: String, trim: true, default: '' },
...
BewerberSchema.index({ standort: 1, status: 1 });
BewerberSchema.index({ email: 1, createdAt: -1 });
