...
class FlipTask {
    constructor(data) {
        ...
        this.due_at = {
            date: data.due_at?.date || null,
            type: data.due_at?.due_at_type || null
        };
        ...
        this.assignment_summary = {
            ...
            items: data.assignment_summary?.items?.map(item => ({
                count: item.count || 0,
    ...
    async find() {
        try {
            ...
            console.log(`🔎 Found ${response.data.tasks.length} tasks matching external_id.`);
            return response.data.tasks.map(task => new FlipTask(task));
    
        } catch (error) {
            console.error("❌ Error fetching Flip Tasks:", error.response ? error.response.data : error.message);
    ...
    async update() {
        try {
            ...
            console.log(`🔄 Updating Flip Task (${this.id}) with:`, JSON.stringify(payload, null, 2));
    
            const response = await flipAxios.patch(`/api/tasks/v4/tasks/${this.id}`, payload, {
            ...
            console.error("❌ Error updating Flip Task:", error.response ? error.response.data : error.message);
            throw new Error(error.response ? JSON.stringify(error.response.data) : error.message);
        }
    }
