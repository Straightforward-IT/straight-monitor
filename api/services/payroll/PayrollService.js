...
const CustomerPayrollRule = require('../../models/Payroll/CustomerPayrollRule');
const AssignmentLedger = require('../../models/Payroll/EinsatzBuch');
const WorkingTimeLedger = require('../../models/Payroll/ArbeitszeitBuch');
const AbsenceLedger = require('../../models/Payroll/AbwesenheitsBuch');
const AZKLedger = require('../../models/Payroll/AZKBuch');
const PayrollAzkDisposition = require('../../models/Payroll/PayrollAzkDisposition');
const PayrollAdjustmentLedger = require('../../models/Payroll/PayrollAdjustmentLedger');
...
const PayrollProviderMapping = require('../../models/Payroll/PayrollProviderMapping');
const PayrollAuditLog = require('../../models/Payroll/PayrollAuditLog');
const PaychexService = require('../integrations/PaychexService');
...
async function syncEmployeeMasterData({ employee, employment, providerProfile, month, companyKey, actor, run = null }) {
  ...
  const employeeHash = require('../integrations/PaychexService').createPayloadHash(employeePayload);
  ...
  const contractHash = require('../integrations/PaychexService').createPayloadHash(contractPayload);
...
async function syncEmployeeAbsences({ run, snapshot, employeeUid, actor }) {
  ...
  for (const absence of absences) {
    ...
    const payloadHash = require('../integrations/PaychexService').createPayloadHash(payload);
    ...
    const plan = require('../integrations/PaychexService').planIdempotentWrite({
...
async function syncPaychex(runId, actor) {
  ...
  for (const snapshot of snapshots) {
    ...
    for (const componentEntry of snapshot.components.filter((entry) => (
      ...
      const idempotencyKey = require('../integrations/PaychexService').buildIdempotencyKey({
