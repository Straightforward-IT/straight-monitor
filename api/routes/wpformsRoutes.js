  ...
  asyncHandler(async (req, res) => {
    ...
    if (type === "laufzettel") {
      ...
      if (mitarbeiterId) {
        ...
        if (mitarbeiter && mitarbeiter.asana_id) {
          ...
          let matchingSubtask = subtasks.find(sub => 
            (sub.name?.includes(formattedDate) && sub.name?.includes(name_teamleiter)) || sub.name?.includes(formattedDate) || sub.name?.includes(name_teamleiter)
          );          
          if (matchingSubtask) {
          ...
          if (!commentResponse || commentResponse.status !== 201) {
            await sendMail(
              ...
              `
                    ...
                    <p><strong>Zieltask:</strong> ${targetGid}</p>
